...
class ScannetDataset(BaseDataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        cache_tag = hashlib.md5(
            f'{os.path.abspath(self.data_root)}|{os.path.abspath(self.scans_root) if self.scans_root else ""}'.encode()
        ).hexdigest()[:10]

        # Cache frame lists per scene. Supports both layouts:
        # 1) scene/color/0.jpg + scene/depth/0.png + scene/pose/0.txt
        # 2) scene/00000.jpg + scene/00000.png + scene/00000.txt (posed_images)
        frame_cache_path = f'data/dataset_cache/scannetmv_{self.dataset_source}_{self.mode}_{cache_tag}_frames_cache.npy'
        if not os.path.exists(frame_cache_path):
            self.scene_frames = {}
            for seq in tqdm(self.sequences, desc="Generating frame cache"):
                self.scene_frames[seq] = self._scan_scene_frames(seq)

            np.save(frame_cache_path, self.scene_frames)
        else:
            self.scene_frames = np.load(frame_cache_path, allow_pickle=True).item()
        self.num_imgs = {seq: len(frames) for seq, frames in self.scene_frames.items()}
        
        # Cache 2D instance annotation paths similarly to num_imgs
        instance_cache_path = f'data/dataset_cache/scannetmv_{self.dataset_source}_{self.mode}_{cache_tag}_instance_path_cache.npy'
        if not os.path.exists(instance_cache_path):
            self.instance_2d_paths = {}
            # Infer the 'scans' path from the data_root
            scans_path = self.scans_root or os.path.join(os.path.dirname(self.data_root), 'scans')
    ...
    def _scan_scene_frames(self, scene):
        ...
        for image_path in image_paths:
            ...
            try:
                ...
                continue
            frames.append({'id': frame_id, 'stem': stem, 'layout': layout})

        frames = sorted(frames, key=lambda x: x['id'])
        if not frames:
            raise ValueError(f"No RGB frames found for scene {scene} in {base_path}")
        return frames

    def _get_intrinsic_path(self, base_path, layout):
        if layout == 'structured':
            return osp.join(base_path, 'intrinsic', 'intrinsic_depth.txt')

        candidates = [
            osp.join(base_path, 'depth_intrinsic.txt'),
            osp.join(base_path, 'intrinsic_depth.txt'),
            osp.join(base_path, 'intrinsic.txt'),
        ]
        for path in candidates:
            if os.path.exists(path):
                return path
        raise FileNotFoundError(f"No intrinsic file found in {base_path}")

    def _get_frame_paths(self, base_path, frame):
        stem = frame['stem']
        if frame['layout'] == 'structured':
            return (
                os.path.join(base_path, 'color', f'{stem}.jpg'),
                os.path.join(base_path, 'depth', f'{stem}.png'),
                os.path.join(base_path, 'pose', f'{stem}.txt'),
            )
        return (
            os.path.join(base_path, f'{stem}.jpg'),
            os.path.join(base_path, f'{stem}.png'),
            os.path.join(base_path, f'{stem}.txt'),
        )
    ...
    def _get_views(self, index, resolution, rng):
        ...
        scene_frames = self.scene_frames[scene]
        frame_by_id = {frame['id']: frame for frame in scene_frames}
        invalid_ids = set(int(i) for i in self.invalid_list.get(scene, []))
        # Exclude invalid frames by the real ScanNet frame id.
        valid_idxs = [frame['id'] for frame in scene_frames if frame['id'] not in invalid_ids]
        if len(valid_idxs) == 0:
        ...
        if idxs is None:
            sampled_relative_indices = np.linspace(
                0, len(valid_idxs) - 1, self.frame_num
            ).round().astype(int)
            idxs = [valid_idxs[i] for i in sampled_relative_indices]
        else:
            idxs = [idx for idx in idxs if idx in frame_by_id]
            if len(idxs) < self.frame_num and valid_idxs:
                sampled_relative_indices = np.linspace(
                    0, len(valid_idxs) - 1, self.frame_num - len(idxs)
                ).round().astype(int)
                idxs.extend([valid_idxs[i] for i in sampled_relative_indices])
        ...
        intrinsic_path = self._get_intrinsic_path(base_path, scene_frames[0]['layout'])
        ...
        for idx in idxs:
            frame = frame_by_id[idx]
            impath, disppath, posepath = self._get_frame_paths(base_path, frame)
